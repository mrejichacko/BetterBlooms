...
sim_comfrey <- simulateResiduals(mod.1_Comfrey)

# Aggregate to unique spatial locations (garden = Id.fac)
sim_comfrey_garden <- recalculateResiduals(sim_comfrey, group = df7e$Id.fac)

# (unique x/y per garden)
sp_test_comfrey <- testSpatialAutocorrelation(
  sim_comfrey_garden,
  x = df7e$X_KOORDINATE[!duplicated(df7e$Id.fac)],
  y = df7e$Y_KOORDINATE[!duplicated(df7e$Id.fac)]
)
