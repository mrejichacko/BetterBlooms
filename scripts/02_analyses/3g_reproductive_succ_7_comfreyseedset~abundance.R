...
#####################################################################
###
### 3g. Pollinator experiment: 
### Modelling plant reproductive success: Comfrey seed set
...
mod.1_Comfrey_seedset <- glmer(cbind(n_seeds, n_unfertilized_ovules) ~ A_Bombus_Comfrey.dayly.1.z + (1|Id.fac/Plant_Id.fac), data=df7f, family="binomial") #A_Bombus_Comfrey.dayly.2.z 
...
mod.2_Comfrey_seedset <- glmer(cbind(n_seeds, n_unfertilized_ovules) ~ S_Bombus_Comfrey.dayly.z + (1|Id.fac/Plant_Id.fac), data=df7f, family="binomial") #A_Bombus_Comfrey.dayly.2.z 
summary(mod.2_Comfrey_seedset) #

r.squaredGLMM(mod.2_Comfrey_seedset)

#mod.2b_Comfrey_seedset <- lmer(N_seeds/4 ~ A_Bombus_Comfrey.dayly.1.z + A_Bombus_Comfrey.dayly.2.z + (1|Id.fac/Plant_Id.fac), data=df7f)
#summary(mod.2b_Comfrey_seedset)

#mod.2c_Comfrey_seedset <- lmer(N_seeds/N_flowers ~ A_Bombus_Comfrey.dayly.1.z + A_Bombus_Comfrey.dayly.2.z + (1|Id.fac/Plant_Id.fac), data=df7f)
#summary(mod.2c_Comfrey_seedset)

#Assessing model assumptions: (check page 144)
par(mfrow=c(2,3))

plot(fitted(mod.2_Comfrey_seedset), resid(mod.2_Comfrey_seedset))
abline(h=0)

qqnorm(resid(mod.2_Comfrey_seedset)) #qq-plot of residuals
qqline(resid(mod.2_Comfrey_seedset))

qqnorm(ranef(mod.2_Comfrey_seedset)$Id.fac[,1]) #qq-plot of the random effects: Id
qqline(ranef(mod.2_Comfrey_seedset)$Id.fac[,1])

qqnorm(ranef(mod.2_Comfrey_seedset)$Plant_Id.fac[,1]) #qq-plot of the random effects: Plant
qqline(ranef(mod.2_Comfrey_seedset)$Plant_Id.fac[,1])

plot(fitted(mod.2_Comfrey_seedset), jitter(df7f$N_seeds/4,0.05))
abline(0,1)    

dev.off() 

#Heteroscedasticity (= nonhomogeneity of the residual variance)
scatter.smooth(df7f$S_Bombus_Comfrey.dayly.z, resid(mod.2_Comfrey_seedset)); abline(0,0, lty=2, col="red")

#Check if random effects are 0:
mean(ranef(mod.2_Comfrey_seedset)$Id.fac[,1])
mean(ranef(mod.2_Comfrey_seedset)$Plant_Id.fac[,1])

#Check for overdispersion:
dispersion_glmer(mod.2_Comfrey_seedset) # bit underdispersed
#######################################################################################################################################
#######################################################################################################################################